...
public:
  ...
  void removeUselessTrackingObjectBoxInfo(vector<int> m_vecUpdate);
private:
  int ivWidth;
  int ivHeight;
...
{ 
  ...
  //  }
  //}
	cout << "ivNObjects:" << ivNObjects << endl;
	for (int i = 0; i < ivCurrentBoxes.size(); i++)
	{
		cout << "IDB:" << i << " " << ivCurrentBoxes[i].objectId << endl;
	}
  ...
  for (int i = 0; i < ivNObjects; i++)
  {
	  if (ivDefined[i])
	  {
		  if (ivCurrentBoxes.size() == 0)
		  {
			  break;
		  }
		  else if (ivCurrentBoxes[i].x<0 || ivCurrentBoxes[i].y<0 ||
			  ivCurrentBoxes[i].x>=ivWidth || ivCurrentBoxes[i].y>=ivHeight ||
			  (ivCurrentBoxes[i].x + ivCurrentBoxes[i].width)>ivWidth ||
			  (ivCurrentBoxes[i].y + ivCurrentBoxes[i].height>ivHeight)||
			  ivWidth<=0||ivHeight<=0)
		  {
			  continue;
		  }
			  ...
			  m_coordinate.ID = ivCurrentBoxes[i].objectId;
...
void MultiObjectTLD::removeUselessTrackingObjectBoxInfo(vector<int> m_vecUpdate)
{
	vector<ObjectBox> m_iCurrenBox;
	///ivCurrentBoxes.clear();
	ObjectBox m_tmpBox;
	m_tmpBox.height = -1;
	m_tmpBox.width = -1;
	m_tmpBox.x = -1;
	...
	int m = 0;
	int m_count = 0;
	int m_vecLen = m_vecUpdate.size();
	int m_iCurrentLen = ivCurrentBoxes.size();

	for (int i = 0; i < m_iCurrentLen; i++)
	{
		bool flag = true;
		for (int j = m; j < m_vecLen; j++)
		{
			if (j == i)
			{
				flag = false;
		...
		if (flag == false)
		{
			...
			//m_iCurrenBox.push_back(m_tmpBox);
		}
		else
		{
			m_iCurrenBox.push_back(ivCurrentBoxes[i]);
			m_iCurrenBox[m_iCurrenBox.size()-1].objectId = m_count;
			m_count++;
			ivNObjects = m_count;
		}
	}
	...
	ivCurrentBoxes.clear();
	for (int i = 0; i < m_iCurrenBox.size(); i++)
	{
		cout << "IDA:" << i << " " << m_iCurrenBox[i].objectId << endl;
		ivCurrentBoxes.push_back(m_iCurrenBox[i]);
	}
	m_iCurrenBox.clear();
}
